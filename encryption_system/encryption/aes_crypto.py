class AES:
    ...
    def aes_ecb_encrypt(self, plaintext, RoundKeys):
        # 对整个明文进行 AES 加密（ECB 模式）
        plaintext = self.pkcs7_padding(plaintext)  # 进行填充
        ciphertext = []
        # 按 16 字节一块进行加密
        for i in range(0, len(plaintext), 16):
            block = plaintext[i:i + 16]
            encrypted_block = self.aes_encrypt(block, RoundKeys)
            ciphertext.extend(encrypted_block)
        return ciphertext

    def aes_ecb_decrypt(self, ciphertext, RoundKeys):
        # 对整个密文进行 AES 解密（ECB 模式）
        plaintext = []
        # 按 16 字节一块进行解密
        for i in range(0, len(ciphertext), 16):
            block = ciphertext[i:i + 16]
            decrypted_block = self.aes_decrypt(block, RoundKeys)
            plaintext.extend(decrypted_block)
        return self.pkcs7_unpadding(plaintext)  # 去除填充
