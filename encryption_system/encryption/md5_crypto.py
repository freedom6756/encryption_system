...
class MD5:
    def __init__(self):
        ...
        self.conn = mysql.connector.connect(
            ...
            user='root',
            password='root',
            database='encryption_system'  # 你的数据库名
        )
    ...
    def check_and_insert(self, message):
        """检查数据库中是否存在该消息的MD5值，如果不存在则插入"""
        hashed_message = self.hash(message)

        # 查询数据库，看是否已有该哈希值
        self.cursor.execute("SELECT original_value FROM md5_table WHERE original_value = %s", (message,))
        result = self.cursor.fetchone()
        if result:
            print("ok")
        else:
            # 插入数据库
            self.cursor.execute("INSERT INTO md5_table (original_value, md5_value) VALUES (%s, %s)",
                                (message, hashed_message))
            self.conn.commit()
            print(f"插入成功！MD5值：{hashed_message}")
        return f"查询成功！MD5值：{hashed_message}"

    def decrypt(self, md5_value):
        """尝试解密MD5值并返回原始消息"""
        self.cursor.execute("SELECT original_value FROM md5_table WHERE md5_value = %s", (md5_value,))
        result = self.cursor.fetchone()
        if result:
            return f"查询成功！原始消息为：{result[0]}"
        else:
            return "抱歉，未找到该MD5值对应的原始消息。"
